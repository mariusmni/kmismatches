...
void lcss() {
	uchar *txt = (uchar*)"cddcdb";
	cout << "Start " << txt << endl;
	int len = strlen((char*)txt) + 1;
	SSTree *st = new SSTree(txt, len);
	st->PrintTree(st->root(), 0);
	cout << "Done" << endl;

	ulong node = st->child(st->root(), 'c');
	cout << st->edge(node) << endl;
	node = st->child(node, 'b');
	cout << st->pathlabel(node) << endl;
	return;

	// A=aaabbbccc, B=ccbbbaa ==> lcss(A,B)=bbb
	char *A = "aaabbbccc", *B = "aaccbbbaa";
	uchar *text = (uchar*) "aaabbbccc$aaccbbbaa";
//	char *A = "ace", *B = "bde";
//	uchar *text = (uchar*) "ace$bde";
	...
	for (i = 0; i <= lastleaf; i++)
		if (sst->isOpen(i) && left[i] == true && right[i] == true
				&& (sst->depth(i) > maxdepth)) {
	...
	cout << "Path labels" << endl;
	for (int i = 0; i < 3*n; ++i) {
		cout << i << " : " << (char*) sst->pathlabel(i) << endl;
	}
	cout << "LCA(11,15) = " << sst->lca(11, 15) << endl;
	cout << "LCE(0,10) = " << sst->lce(0, 10) << endl;
	cout << "TextPos(32) = " << sst->textpos(32) << endl;
	delete sst;
	delete left;
	delete right;
