 ...
 */
SSTree::SSTree(uchar *text, ulong n, bool deletetext, unsigned samplerate,
		io_action IOaction, const char *filename) {
#ifdef SSTREE_HEAPPROFILE
	ulong heapCon;
#endif
	this->n = n;
	...
	if (IOaction == load_from && filename != 0)
		...
		sa = new CSA(text, n, floorLog2n, 0,
				(string(filename) + ".csa").c_str());
	else
		// No IO operation
	...
	ulong bitsInP;
	if (IOaction == load_from && filename != 0)
		P = LcpToParentheses::GetBalancedParentheses(
				(string(filename) + ".bp").c_str(), bitsInP);
	else
		P = LcpToParentheses::GetBalancedParentheses(hgt, n, bitsInP);

	if (IOaction == save_to && filename != 0)
		LcpToParentheses::SaveToFile((string(filename) + ".bp").c_str(), P,
				bitsInP);
#ifdef SSTREE_TIMER
	...
	fflush(stdout);
#endif
	rmq = new CRMQ(br, P, bitsInP, rmqSampleRate * rmqSampleRate
			* rmqSampleRate, rmqSampleRate * rmqSampleRate, rmqSampleRate);
#ifdef SSTREE_TIMER
#ifdef SSTREE_HEAPPROFILE
	std::cout << "--> HeapProfiler: " << HeapProfiler::GetHeapConsumption() - heapCon << ", " << HeapProfiler::GetMaxHeapConsumption() << std::endl;
#endif
...
void SSTree::edge(ulong v, ulong &pstart, ulong &len) {
	if (isleaf(v)) {
		ulong i = leftrank(v);
		ulong j = depth(parent(v));
		ulong k = depth(v);
	...
	for (w = 1; w < len - 1; w++) {
		...
		while (v1 < len) {
			if (lca(temp, v1) != lcaParen(temp, v1)) {
				printf(
						"conflict at (%lu, %lu)::lcaParen() = %lu and lca() = %lu\n",
...
ulong SSTree::search(uchar *pattern, ulong l) {
	...
	// Get rank_1's
	ulong r_sp = br->rank(sp), r_ep = br->rank(ep);
	// Calculate number of open and closed parentheses on the interval [sp, ep]
	ulong open = r_ep - r_sp + 1, close = (ep - r_ep) - (sp - r_sp);
